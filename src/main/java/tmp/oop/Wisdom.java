...
public abstract class Wisdom implements Comparable<Wisdom> {
   ...
   *
   * @param scan источник данных
   * @throws NumberFormatException  если в строке указано не byte значение, либо величина не
  ...
  public void inRate(Scanner scan) throws NumberFormatException, NoSuchElementException {
    ...
    try {
      ...
      throw new NumberFormatException(
          "Incorrect rate input: Expected: [1 - 10]. Received: " + line);
    }
    //Проверка на соответствие установленным значениям оценки мудрости
    if (rate < 1 | rate > 10) {
      rate = 0;
      throw new NumberFormatException(
          "Incorrect rate value: Expected: [1 - 10]. Received: " + line);
    }
  }

  /**
   * Абстрактный метод, в котором должны описываться действия для вывода пары мудростей Тело метода
   * будет зависеть от того, от какой мудрости вызван метод
   *
  ...
  /**
   * Абстрактный метод с описанием действий для вывода информации о паре мудростей, где одна из
   * мудростей Aphorism, вторая - та, из экземпляра которой был вызван метод
   *
   * @param pw источник для записи информации о паре
   */
  ...
  /**
   * Абстрактный метод с описанием действий для вывода информации о паре мудростей, где одна из
   * мудростей Proverb, вторая - та, из экземпляра которой был вызван метод
   *
   * @param pw источник для записи информации о паре
   */
  ...
  /**
   * Абстрактный метод с описанием действий для вывода информации о паре мудростей, где одна из
   * мудростей Riddle, вторая - та, из экземпляра которой был вызван метод
   *
   * @param pw источник для записи информации о паре
   */
  ...
  long countPunctuationMarks() {
    ...
    String punMarks = "!.,:;'-?\"";
    return text.chars().filter((x) -> punMarks.indexOf(x) != -1)
        .count(); //Удаление из потока всех символов, не входящими в строку punMarks и вывод их количества
  }

  /**
   * Реализация метода, необходимого для наследования интерфейса Compareable, позволяющего
   * сравнивать 2 экземпляра данного класса по величине. Для сравнения подсчитывается количество
   * знаков препинания
   *
